...
const getping = ("/ping", async (req, res) => {
  const database = await acceso.query("SELECT 1 + 1").then(() => "Servidor está levantado.") //        .catch(() => console.log(acceso))
  .catch(() => "Servidor está abajo.");
  res.status(200).json(database);
}); //Utilidades

const getesas = async (req, res) => {
...
const getAllesas = async (req, res) => {
  const body = JSON.parse(req.query['Z']);
  const validaT = verificaToken(body);
...
const deleteesas = async (req, res) => {
  const body = JSON.parse(req.query['Z']);
  const validaT = verificaToken(body);
  const tokenDecoder = JSON.parse(decodificaToken(body));
  const tokenAcceso = tokenDecoder['token'];
  ...
  if (validaT) {
    const preparaJson = JSON.parse('{"idusuario": "' + tokenDecoder['idusuario'] + '","idesa":"' + tokenDecoder['idemp'] + '", "tokencceso": "' + tokenAcceso + '"}');
    const valToken = await acceso.query('select public.gettokenusers($1)', [preparaJson]);
    const puedeGrabar = eval(valToken.rows[0]['gettokenusers']['ret']);
...
async function validaAcceso(idesa, idusuario) {
  try {
    ...
    const dato = await acceso.query('select public.getaccesosistema($1)', [JSON.parse(val)]).then(r => {
      return JSON.stringify(r.rows[0]['getaccesosistema']);
